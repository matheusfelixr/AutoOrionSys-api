package com.flexsys.config;

import com.flexsys.entity.*;
import com.flexsys.repository.*;
import com.flexsys.entity.PerfilAcesso;
import com.flexsys.repository.PerfilAcessoRepository;
...
public class DataInitializer {
    ...
    CommandLineRunner initData(
        ...
        return args -> {
            ...
            String senhaDefault = passwordEncoder.encode("flexsys123");

            // ── Usuários ─────────────────────────────────────────────────────
            usuarios.save(Usuario.builder().nome("Ana Souza").email("ana.souza@flexsys.com.br")
            ...
            usuarios.save(Usuario.builder().nome("Carlos Mendes").email("carlos.mendes@flexsys.com.br")
            ...
            usuarios.save(Usuario.builder().nome("Fernanda Lima").email("fernanda.lima@flexsys.com.br")
            ...
            usuarios.save(Usuario.builder().nome("Ricardo Alves").email("ricardo.alves@flexsys.com.br")
                    .senha(senhaDefault).cargo("Técnico de Campo").perfil(Usuario.PerfilUsuario.tecnico)
                    .status(Usuario.StatusUsuario.ativo).dataCadastro(LocalDateTime.now()).build());

            usuarios.save(Usuario.builder().nome("Camila Santos").email("camila.santos@flexsys.com.br")
            ...
            parametros.save(Parametro.builder().nome("prmNomeSistema").descricao("Nome exibido no sistema")
                    .grupoId(gpGeral.getId()).valor("FlexSys Demo").tipo("texto").ordem(1).build());
            ...
            var adminDemo = usuarios.findByEmail("ana.souza@flexsys.com.br").orElse(null);
            if (adminDemo != null) {
                notificacoes.save(Notificacao.builder()
                        .usuarioId(adminDemo.getId())
                        .titulo("Bem-vindo ao FlexSys!")
